...
gpomol_pipeline.py
==================
ONE script, ONE shared molecule subsample, ONE collection-pass ordering guarantee.

Modes (each reuses the same collection + referencing where applicable):
  * Wasserstein diagnostic (legacy)      : default, and --sweep
  * WL / SOAP variogram screen           : --wl-screen        [--descriptor wl|soap]
  * Wendland support-radius sweep        : --wl-support-sweep [--descriptor wl|soap]
  * support-sweep comparison overlays    : --wl-sweep-compare {mean-function,elements,n}
  * WL conformer-blindness scan          : --wl-conformer-scan
  * system-id conformer scan             : --sid-scan

The alignment guarantee: every per-molecule quantity (features, target, sizes,
charges) is appended in ONE pass in ONE order, so row i of any distance/kernel
matrix and entry i of the target are always the same molecule -- there is no
index map to get wrong.

Requirements
------------
    pip install ase numpy matplotlib fairchem-core
    pip install dscribe          # only for --descriptor soap
    pip install pot              # only for the exact-EMD Wasserstein path
Sibling modules (same directory): variogram_screen.py, wl_kernel.py,
mean_function.py, soap_kernel.py, conformer_scan.py, sid_scan.py.
"""
import argparse
import os
import sys
...
try:
    import ot  # POT: exact EMD (Wasserstein path only)
    _HAVE_POT = True
except Exception:
    _HAVE_POT = False
...
def collect_subset(dataset, n_molecules, pool, size_range, mode, seed,
    ...
    so geometry (for W / WL / SOAP) and energy (for y) can never desync."""
...
# =========================================================================== #
#  PART 1 -- Option-4 representation + Wasserstein distance matrix (legacy)
# =========================================================================== #
...
def _pair_block(reps, block, alpha, beta, p, use_pot):
    return [(i, j, molecule_distance(reps[i], reps[j], alpha, beta, p, use_pot))
            for (i, j) in block]
...
def wasserstein_matrix(reps, alpha, beta, p, use_pot=True, n_jobs=1):
    ...
    if n_jobs == 1:
        ...
        block_size = max(1, -(-len(pairs) // n_blocks))
...
# =========================================================================== #
#  PART 2 -- per-element referencing (legacy Wasserstein target)
# =========================================================================== #
...
# =========================================================================== #
#  PART 3 -- exp-kernel diagnostic (legacy Wasserstein readouts)
# =========================================================================== #
...
def print_readout(n, median_w, eps, labels, rows, pd_v, worst,
    ...
    out = []; a = out.append
    a("=" * 72); a("  GPOMOL UNIFIED PIPELINE READOUT"); a("=" * 72)
    ...
    for lab, v in zip(labels, eps):
        a(f"      {lab:>14} : {v:12.3f}")
    a(""); a("-" * 72); a("  (A) IS exp(-W/ell) PD ON THIS DATA?"); a("-" * 72)
    ...
    a(""); a("-" * 72); a("  (B) DOES W CARRY THE ENERGY SIGNAL?  [kernel-free]"); a("-" * 72)
    ...
    a("-" * 72); a(f"  VERDICT (B): {'SIGNAL PRESENT' if signal else 'NO USABLE SIGNAL'}")
    a("-" * 72); a("=" * 72)
    print("\n".join(out))
...
def dedupe_combos(combos):
    ...
    for (a, b, p) in combos:
        ...
        if key in seen:
            continue
        seen.add(key); unique.append((a, b, p))
    return unique
...
def run_sweep(reps, y, agrid, bgrid, pgrid, use_pot, n_jobs, no_gp):
    ...
    print(f"sweep: {len(requested)} requested -> {len(unique)} distinct metric(s)\n")
    results, Ws = [], {}
    for k, (a, b, p) in enumerate(unique):
        ...
        results.append({
            ...
            "median_w": d["median_w"], "pd_verdict": d["pd_verdict"],
            "pd_worst": d["pd_worst"], "spearman": d["de"]["spearman"],
            "lift": d["de"]["lift"], "knn_k": d["knn"]["best"]["k"],
            "knn_skill": d["knn"]["best"]["skill"],
...
def print_sweep_table(results):
    ...
    print("\n" + "=" * 88)
    print("  ALPHA / BETA / P SWEEP  (sorted by k-NN skill)")
    print("=" * 88)
    print(f"  {'alpha':>6} {'beta':>6} {'p':>3} {'a:b':>9} {'PD':>11} {'min/max':>10} "
...
def make_sweep_plot(results, outdir, ts):
    ...
    skills = [r["knn_skill"] for r in rows]; rhos = [r["spearman"] for r in rows]
    ...
    ax[0].bar(x, skills, color=colors); ax[0].axhline(0, color="k", lw=0.8)
    ax[0].set(title="best k-NN skill per metric", ylabel="skill = 1 - MAE/MAE_base")
    ax[1].bar(x, rhos, color=colors); ax[1].axhline(0, color="k", lw=0.8)
...
def make_demo(n=120, shuffle_energy=False, seed=0):
    """Synthetic molecules whose energy residual depends on molecular SCALE."""
    rng = np.random.default_rng(seed)
    elems = ["H", "C", "N", "O", "S"]
    ...
    for _ in range(n):
        ...
        energies.append(comp_e + 6.0 * scale + 0.2 * rng.normal())
    ...
    return {"indices": np.arange(n), "symbols": symbols, "positions": positions,
            ...
            "charges": np.array(charges), "spins": np.zeros(n, dtype=int)}, exp


# =========================================================================== #
#  WL / SOAP screening + support sweeps (descriptor-agnostic)
# =========================================================================== #
def make_candidate(name, args, normalize):
    """Build the primary screening candidate for the chosen descriptor."""
    if args.descriptor == "soap":
        from soap_kernel import make_soap_candidate
        return make_soap_candidate(name, rcut=args.soap_rcut, nmax=args.soap_nmax,
                                   lmax=args.soap_lmax, sigma=args.soap_sigma,
                                   zeta=args.soap_zeta, normalize=normalize)
    from wl_kernel import make_wl_candidate
    return make_wl_candidate(name, h=args.wl_sweep_depth, scale=args.wl_scale,
                             normalize=normalize)
...
def sweep_one(feats, y, cand, noise_frac):
    """Core support-radius sweep on ONE (feats, y) with a prebuilt candidate:
    returns (rows, variogram_range). Shared by the single sweep and every
    comparison axis so they can't diverge."""
    ...
    rows = [m for r in radii
            if (m := wendland_loo_crps(D, float(r), y, noise_frac=noise_frac))["ok"]]
    return rows, float(vg.vrange)


def run_wl_screen(data, y, args, ts):
    """Variogram + feature-kernel screen for the chosen descriptor, reusing the
    SAME subsample and referenced target y (one collection-pass order)."""
    ...
    if args.descriptor == "soap":
        cands = [make_candidate("soap", args, normalize=False)]
    else:
        from wl_kernel import make_wl_candidate
        cands = []
    ...
    print(f"\n=== {args.descriptor.upper()} VARIOGRAM SCREEN (as a DISTANCE -> Wendland) ===")
    print(f"  feats={len(feats)}  referenced-y spread={np.ptp(y):.3g} eV  "
          f"sizes {sizes.min()}-{sizes.max()} atoms\n")
    ...
    print("\n=== CONTRAST: the SAME kernel used as a FEATURE (dot-product) GP ===\n")
...
def run_wl_support_sweep(data, y, args, ts):
    """Sweep the Wendland support radius on ONE descriptor kernel and trace
    GP-LOO CRPS vs matrix density -- the signal-vs-sparsity operating point."""
    from variogram_screen import print_sweep_summary, summarize_support_sweep
    feats = _feats_from_data(data)
    normalize = (args.wl_sweep_normalize == "norm")
    tag = args.descriptor + ("_norm" if normalize else "_raw")
    cand = make_candidate(tag, args, normalize)

    print(f"\n=== SUPPORT-RADIUS SWEEP  ({tag}, mean={args.mean_function}) ===")
    rows, vrange = sweep_one(feats, y, cand, args.noise_frac)
    print(f"  built {args.descriptor} distance for {len(feats)} molecules   "
          f"variogram range={vrange:.3f}")
    ...
    plot = _plot_support_sweep(rows, vrange, s, args.outdir, ts, tag)
...
def _plot_support_sweep(rows, vrange, s, outdir, ts, tag):
    ...
    ax[0].set(xscale="log", xlabel="matrix density (off-diag nonzero fraction)",
              ylabel="GP-LOO CRPS (eV)", title=f"signal vs sparsity: {tag}")
...
def run_wl_sweep_compare(args, ts):
    ...
    Descriptor-agnostic via make_candidate()."""
    ...
    def target(data, basis):
        return compute_target(data, basis=basis, scale=args.wl_scale,
                              add_charge=args.add_charge,
                              add_intercept=args.add_intercept)["g"]

    if args.wl_sweep_compare == "mean-function":
        data = _collect_for_compare(args, args.elements, args.n_molecules)
        feats = _feats_from_data(data)
        cand = make_candidate(args.descriptor, args, normalize)
        for basis in args.compare_bases:
            rows, vr = sweep_one(feats, target(data, basis), cand, nf)
        ...
        for em in args.compare_elements:
            ...
            feats = _feats_from_data(data)
            cand = make_candidate(args.descriptor, args, normalize)
            rows, vr = sweep_one(feats, target(data, args.mean_function), cand, nf)
            curves.append((f"{em} (N={len(feats)})", rows, vr,
                           summarize_support_sweep(rows)))
        ...
        for n in sorted(args.compare_n):
            ...
            cand = make_candidate(args.descriptor, args, normalize)
            rows, vr = sweep_one(feats_all[:n], y_all[:n], cand, nf)
    ...
    for label, rows, vr, s in curves:
        ...
        print_sweep_summary(s)
    plot = _plot_sweep_overlay(curves, args.wl_sweep_compare, args.outdir, ts,
                               args.descriptor)
    print(f"\nsaved -> {plot}")


def _plot_sweep_overlay(curves, axis, outdir, ts, descriptor):
    ...
    ax[0].set(xscale="log", xlabel="matrix density (off-diag nonzero fraction)",
              ...
              title=f"CRPS vs sparsity ({descriptor});  * = optimum")
    ax[0].legend(fontsize=8)
...
def run_sid_scan(args, ts):
    ...
    for k in range(n):
        ...
        charges.append(float(q)); comp_sig.append(tuple(sorted(c.items())))
        kept += 1
...
def main():
    ...
    ap.add_argument("--charge", type=int, default=None, help="keep only this formal charge")
    ap.add_argument("--spin", type=int, default=None, help="keep only this spin multiplicity")
    ap.add_argument("--seed", type=int, default=0)
    # Option-4 / Wasserstein (legacy)
    ap.add_argument("--alpha", type=float, default=1.0)
    ap.add_argument("--beta", type=float, default=1.0)
    ap.add_argument("--wasserstein-p", type=int, default=2)
    ap.add_argument("--quantile-points", type=int, default=16)
    ...
    # mean function (WL / SOAP target)
    ap.add_argument("--mean-function", choices=["element", "element+bonds"],
                    default="element")
    # descriptor selection
    ap.add_argument("--descriptor", choices=["wl", "soap"], default="wl",
                    help="which descriptor the screen / sweep uses")
    ap.add_argument("--soap-rcut", type=float, default=5.0)
    ap.add_argument("--soap-nmax", type=int, default=6)
    ap.add_argument("--soap-lmax", type=int, default=4)
    ap.add_argument("--soap-sigma", type=float, default=0.5)
    ap.add_argument("--soap-zeta", type=int, default=1)
    # WL screen
    ...
    ap.add_argument("--sid-key", default="sid")
    ap.add_argument("--sid-pool", type=int, default=60000)
    ap.add_argument("--sid-start", type=int, default=0)
    # io
    ...
    # --- modes that collect their OWN data (before the shared collection) ---
    ...
    # --- WL / SOAP modes: target from the mean-function decomposition ---
    ...
    # --- legacy Wasserstein target (per-element referencing) ---
    ...
    assert len(reps) == y.shape[0]

    # ---- Wasserstein SWEEP mode ----
    if args.sweep:
    ...
    # ---- Wasserstein SINGLE-METRIC mode ----
